...
def train_LightGBM_model(X,Y):
    ...
    train_data = lgb.Dataset(X, Y,categorical_feature=[0,1,2,3,4657,4658,4659,4660])
...
def train_models(target):
    ...
    for i in range(5):
        ...
        plt.savefig('importance_scatter_'+target+'_'+str(i)+'_lgb.pdf', format='pdf', dpi=1200, bbox_inches='tight')
        plt.close()
...
def SHAP_analysis_moa(target):
    for i in range(5):
        ...
        test_path = '../test_by_batch/fold_'+str(i)+'/moa/*.tsv'
        ...
        for path in all_moa_path:
            try:
                ...
                shap_values = shap.TreeExplainer(regressor).shap_values(Test_X)
                shap.summary_plot(shap_values, Test_X, show=False)
                plt.savefig('./moa/'+name+'/importance_scatter_'+target+'_'+str(i)+'_lgb.pdf', format='pdf', dpi=1200, bbox_inches='tight')
                plt.close()
...
def Dependency_plot_of_genes(target,moa ='ATMi_ATRi', gene = 'PARP1'):
    for i in range(5):
        try:
            ...
            regressor = pickle.load(open(filename, 'rb'))
            moa_path = '../test_by_batch/fold_'+str(i)+'/'+moa+'.tsv'
